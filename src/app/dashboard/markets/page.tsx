...
// function pairRows(prices: Record<string, PriceData>, search: string, sort: SortKey) {
//   console.log('Calculating pair rows with', { prices, search, sort }) // Debug log
//   return PAIRS.filter((pair) => pair.toLowerCase().includes(search.toLowerCase()))
//     .map((pair) => {
//       const ticks = EXCHANGE_IDS.map((exchange) => prices[`${exchange}:${pair}`]).filter(Boolean)
//       const cheapestAsk = ticks.reduce<PriceData | null>((best, tick) => !best || tick.ask < best.ask ? tick : best, null)
//       const highestBid = ticks.reduce<PriceData | null>((best, tick) => !best || tick.bid > best.bid ? tick : best, null)

//       const spreadData = cheapestAsk && highestBid && cheapestAsk.exchange !== highestBid.exchange
//         ? calculateNetSpread(
//             cheapestAsk.ask,
//             highestBid.bid,
...
//         volume: ticks.reduce((sum, tick) => sum + (tick.volume24h ?? 0), 0),
//         change: ticks[0]?.lastPrice ? ((ticks[0].lastPrice - (ticks[0].low24h ?? ticks[0].lastPrice)) / ticks[0].lastPrice) * 100 : 0,
//       }
//     })
//     .sort((left, right) => {
//       if (sort === 'spread') return right.netSpread - left.netSpread
...
//   sort: SortKey
// ) {
//   console.log('Calculating pair rows with', { prices, search, sort })

//   // Dynamically get pairs from websocket data
//   const dynamicPairs = Array.from(
//     new Set(
//       Object.values(prices).map((price) => price.pair)
//     )
//   )
...
//     )
//     .map((pair) => {
//       const ticks = EXCHANGE_IDS
//         .map((exchange) => prices[`${exchange}:${pair}`])
//         .filter(Boolean)

//       const cheapestAsk = ticks.reduce<PriceData | null>(
//         (best, tick) =>
//           !best || tick.ask < best.ask ? tick : best,
//         null
//       )

//       const highestBid = ticks.reduce<PriceData | null>(
//         (best, tick) =>
//           !best || tick.bid > best.bid ? tick : best,
//         null
//       )
...
//               highestBid.bid,
//               EXCHANGES[
//                 cheapestAsk.exchange.toLowerCase() as keyof typeof EXCHANGES
//               ].fee,
//               EXCHANGES[
//                 highestBid.exchange.toLowerCase() as keyof typeof EXCHANGES
//               ].fee
//             )
//           : { gross: 0, fees: 0, net: 0 }
...
//     .sort((left, right) => {
//       if (sort === 'spread')
//         return right.netSpread - left.netSpread
